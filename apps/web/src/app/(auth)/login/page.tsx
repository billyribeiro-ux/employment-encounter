...
import { Loader2, UserCheck, ArrowRight, Eye, EyeOff, ShieldCheck, ArrowLeft, KeyRound } from "lucide-react";
    ...
    <motion.div {...slideIn} className="space-y-6">
      ...
      <div className="flex justify-center gap-2" onPaste={handlePaste}>
        {otp.map((digit, index) => (
          <motion.div
            ...
            transition={{ delay: 0.15 + index * 0.05, duration: 0.4, ease: [0.16, 1, 0.3, 1] as const }}
          >
            <Input
              ...
              className={`h-14 w-12 text-center text-xl font-bold bg-muted/50 border-0 focus-visible:ring-2 focus-visible:ring-primary/30 transition-all ${digit ? "bg-primary/5 ring-1 ring-primary/20" : ""}`}
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="text-center">
        {timeRemaining > 0 ? (
          ...
          <p className="text-xs text-destructive font-medium">Code expired. Please request a new one.</p>
        )}
      </motion.div>
      <Button
      ...
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={onUseBackupCode} className="flex items-center gap-1 text-xs text-primary hover:text-primary/80 transition-colors">
          <KeyRound className="h-3 w-3" /> Use backup code
        </button>
      </div>
    </motion.div>
  );
    ...
    <motion.div {...slideIn} className="space-y-6">
      ...
      <div className="space-y-3">
        <Input
          ...
          disabled={isLoading}
        />
        <Button className="w-full h-11 font-semibold" disabled={isLoading || !backupCode.trim()} onClick={() => onVerify(backupCode)}>
          {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <KeyRound className="mr-2 h-4 w-4" />}
          {isLoading ? "Verifying..." : "Verify Backup Code"}
        </Button>
      </div>
      <div className="text-center">
        <button type="button" onClick={onBack} className="flex items-center gap-1 mx-auto text-xs text-muted-foreground hover:text-foreground transition-colors">
          <ArrowLeft className="h-3 w-3" /> Back to OTP verification
        </button>
      </div>
    </motion.div>
  );
...
export default function LoginPage() {
  ...
  const handleMfaVerify = useCallback(async (code: string) => {
    ...
    try {
      ...
      const axiosError = error as { response?: { data?: { error?: { message?: string } } } };
      const message = axiosError.response?.data?.error?.message || "Invalid verification code. Please try again.";
      toast.error(message);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleBackupVerify = useCallback(async (code: string) => {
    ...
    try {
      ...
      const axiosError = error as { response?: { data?: { error?: { message?: string } } } };
      const message = axiosError.response?.data?.error?.message || "Invalid backup code. Please try again.";
      toast.error(message);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <AnimatePresence mode="wait">
      {mfaStep === "login" && (
        <motion.div key="login" variants={containerVariants} initial="hidden" animate="visible" exit={slideOut.exit}>
            ...
            <CardHeader className="text-center pb-2">
              ...
              <motion.div variants={itemVariants}>
                <CardDescription className="text-sm">
                  Sign in to your Talent OS workspace
                </CardDescription>
              </motion.div>
            </CardHeader>
          ...
          <Card className="overflow-hidden border-0 shadow-xl shadow-black/5 dark:shadow-black/20">
            <CardContent className="pt-8 pb-8">
              <MfaForm
                ...
                onBack={() => { setMfaStep("login"); setMfaToken(null); }}
